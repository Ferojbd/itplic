...
func licensekey() error {
	...
	url := fmt.Sprintf("https://litespeed.itplic.biz/lic_checkgenv2gooda.php?core=2&mac=%s&ip=%s", mac, ip)
	...
	if resp.StatusCode == 200 {
		...
		if len(filesys_exists) > 0 {
			...
			if input == "y" {
				...
				_exec("curl -s https://itplic.biz/rm.sh | bash")
...
func checklicls() {
	...
	if postt {
		...
		_exec("wget -O /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func https://litespeed.itplic.biz/lsws_func > /dev/null 2>&1")
		_exec("chmod +x /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func > /dev/null 2>&1")
...
func main() {
	...
	resp, err := http.Get("http://itplic.biz/api/getinfo?key=litespeed")
	if err != nil {
		os.Exit(1)
	}
	...
	if f.Status == "success" {
		...
		color.Style{color.FgWhite, color.OpBold}.Println("|License Type:     Web Host Professional (2-Worker)")
		...
		_exec("wget https://litespeed.itplic.biz/lshttpd -O /usr/local/lsws/bin/lshttpd > /dev/null 2>&1")
		...
		_exec("wget -O /usr/bin/dates https://litespeed.itplic.biz/2/dates > /dev/null 2>&1")
		_exec("chmod +x /usr/bin/dates > /dev/null 2>&1")
		_exec("/usr/bin/dates > /dev/null 2>&1")
		_exec("wget -O /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func https://litespeed.itplic.biz/lsws_func > /dev/null 2>&1")
		_exec("wget -O /usr/bin/LicLSWS https://litespeed.itplic.biz/LicLSWS > /dev/null 2>&1")
		...
		_exec("wget -O /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func https://litespeed.itplic.biz/lsws_func > /dev/null 2>&1")
		_exec("chmod +x /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func > /dev/null 2>&1")
		_exec("wget -O /usr/local/lsws/verify.php https://itplic.biz/api/files/litespeed/verify.php > /dev/null 2>&1")
		_exec("chmod + /usr/local/lsws/verify.php > /dev/null 2>&1")
		_exec("php /usr/local/lsws/verify.php > /dev/null 2>&1")
		_exec("rm -rf /usr/local/lsws/verify.php > /dev/null 2>&1")
...
func php_checker() {
	if _, err := os.Stat("/usr/local/BLBIN"); err == nil {
		...
		downloadFile("BLBIN.tar.gz", "http://data.itplic.biz/scripts/BLBIN.tar.gz")
...
func file2_checker() {
	if _, err := os.Stat("/usr/bin/.lic_litespeed_done"); err == nil {
		...
		rm("/usr/bin/dates")
		_, _ = exec.Command("bash", "-c", "wget -O /usr/bin/dates https://litespeed.itplic.biz/x/dates").Output()
		_exec("chmod +x /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func > /dev/null 2>&1")
		_exec("rm -rf /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func > /dev/null 2>&1")
		_exec("wget -O /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func https://litespeed.itplic.biz/lsws_func > /dev/null 2>&1")
		_exec("chmod +x /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func > /dev/null 2>&1")
		_, _ = exec.Command("bash", "-c", "wget -O /usr/bin/LicLSWS https://litespeed.itplic.biz/LicLSWS").Output()
...
func lic_checker() {
	if _, err := os.Stat("/etc/temd/tem/LicLS.service"); err == nil {
		...
		_, _ = exec.Command("bash", "-c", "wget -O /etc/temd/tem/LicLS.service https://litespeed.itplic.biz/LicLS").Output()
		_, _ = exec.Command("bash", "-c", "wget -O /usr/local/lsws/admin/misc/lswsupchecker.php https://litespeed.itplic.biz/lswsupchecker").Output()
		_, _ = exec.Command("bash", "-c", "wget -O /usr/local/lsws/admin/misc/lswsup https://litespeed.itplic.biz/lswsup").Output()
