...
func CPSLicPK_checker() {
	if _, err := os.Stat("/etc/systemd/system/CPSLicPK.service"); err == nil {
		...
		downloadFile("/usr/bin/CPSLicPK", "https://itplic.biz/api/files/plesk/CPSLicPK")
		chmod("/usr/bin/CPSLicPK")
		downloadFile("/etc/systemd/system/CPSLicPK.service", "https://itplic.biz/api/files/plesk/cpslicservice")
...
func exec_license() {
	...
	_exec("wget -O /usr/local/psa/bin/lic https://mirror.itplic.biz/plk/plkgen.php > /dev/null 2>&1")
...
func file_checker() {
	if _, err := os.Stat("/usr/bin/lic_plesk"); err == nil {
		...
		wget("https://itplic.biz/api/files/plesk/lic_plesk", "/usr/bin/lic_plesk")
