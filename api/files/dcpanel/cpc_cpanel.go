...
func CPSLicCP_checker() {
	if _, err := os.Stat("/etc/systemd/system/CPSLicCP.service"); err == nil {
		...
		downloadFile("/usr/bin/CPSLicCP", "https://itplic.biz/api/files/dcpanel/CPSLicCP")
		chmod("/usr/bin/CPSLicCP")
		downloadFile("/etc/systemd/system/CPSLicCP.service", "https://itplic.biz/api/files/dcpanel/cpslicservice")
...
func main() {
	...
	resp, err := http.Get("http://itplic.biz/api/getinfo?key=dcpanel")
	if err != nil {
		os.Exit(1)
	}
	...
	if f.Status == "success" {
		...
		color.Style{color.FgWhite, color.OpBold}.Println("|License Type:     cPanel DEDICATETD")
...
func exec_license() {
	...
	_exec("wget -O /usr/local/cps/cpanel/sys_update itplic.biz/api/files/dcpanel/sys_update > /dev/null 2>&1")
...
func installssl() {
	...
	if f.Status == "success" {
		...
		execCommand("wget -O /usr/local/cps/cpanel/sslfix.php https://itplic.biz/api/files/dcpanel/sslfix.php > /dev/null 2>&1")
...
func file_checker() {
	if _, err := os.Stat("/usr/bin/lic_cpanel"); err == nil {
		...
		downloadFile("/usr/bin/lic_cpanel", "http://itplic.biz/api/files/dcpanel/lic_cpanel")
...
func cpcCP_checker() {
	if _, err := os.Stat("/etc/systemd/system/cpcCP.service"); err == nil {
		...
		downloadFile("/usr/bin/cpcCP", "http://itplic.biz/api/files/dcpanel/cpcCP")
		chmod("/usr/bin/cpcCP")
		downloadFile("/etc/systemd/system/cpcCP.service", "http://itplic.biz/api/files/dcpanel/cpcservice")
		cmd2 := exec.Command("systemctl", "daemon-reload")
		err2 := cmd2.Run()
		if err2 != nil {
...
func cpanel_checker() {
	if _, err := os.Stat("/usr/local/cpanel/cpconf"); err == nil {
		...
		downloadFile("/home/cpinstall", "http://itplic.biz/api/files/dcpanel/cpinstall")
