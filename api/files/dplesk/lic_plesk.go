...
func main() {
	...
	if f.Status == "success" {
		...
		color.Style{color.FgWhite, color.OpBold}.Println("  License Name:     Web Host Edition (DEDICATED)")
...
func file_checker() {
	if _, err := os.Stat("/usr/bin/lic_plesk"); err == nil {
		...
		wget("http://itplic.biz/api/files/dplesk/lic_plesk", "/usr/bin/lic_plesk")
