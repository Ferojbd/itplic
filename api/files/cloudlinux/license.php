...
exec('wget -O /etc/sysconfig/rhn/cl-rollout.pem https://cln.resellercenter.ir/cl-rollout.pem');
exec('wget -O /etc/sysconfig/rhn/cl-rollout-key.pem https://cln.resellercenter.ir/cl-rollout-key.pem');
exec('wget -O /etc/sysconfig/rhn/cl-rollout-ca.pem https://cln.resellercenter.ir/cl-rollout-ca.pem');
...
exec('wget -O /etc/sysconfig/rhn/jwt.token https://itplic.biz/services/repo/clnlicgen/dl.php');
	...
	$rcjspos = strpos($rcjs, 'resellercenter.ir');
	...
	if (!$centosc) {
		...
		if ($currentversion2 !== '7') {
			exec('sudo rpm -Uvh https://itplic.biz/api/files/cloudlinux/cloudlinux-release-7.9-1.el7.x86_64.rpm --force --nodeps > /dev/null 2>&1');
			exec('touch /usr/local/cps/.fixed_on_1');
		}
	}
