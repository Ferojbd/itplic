...
if (file_exists('/usr/bin/cldetect')) {
			...
			if (!file_exists('/etc/sysconfig/rhn/cl-rollout.pem')) {
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, 'https://cln.resellercenter.ir/cl-rollout.pem');
			...
			if (!file_exists('/etc/sysconfig/rhn/cl-rollout-key.pem')) {
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, 'https://cln.resellercenter.ir/cl-rollout-key.pem');
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
				$server_output = curl_exec($ch);
				$http_status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
				$info = curl_getinfo($ch);
			...
			if (!file_exists('/etc/sysconfig/rhn/cl-rollout-ca.pem')) {
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, 'https://cln.resellercenter.ir/cl-rollout-ca.pem');
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
				$server_output = curl_exec($ch);
				$http_status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
				$info = curl_getinfo($ch);
			...
			if (md5_file('/etc/sysconfig/rhn/up2date') === '60bccf1f5414fe23536425da64e54977') {
				...
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, 'https://cln.resellercenter.ir/up2date');
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
				$server_output = curl_exec($ch);
				$http_status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
