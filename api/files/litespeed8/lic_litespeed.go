...
func licensekey() error {
	...
	url := fmt.Sprintf("https://litespeed.itplic.biz/lic_checkgenv2gooda.php?core=8&mac=%s&ip=%s", mac, ip)
...
func main() {
	...
	resp, err := http.Get("http://itplic.biz/api/getinfo?key=litespeed8")
	if err != nil {
		os.Exit(1)
	}
	...
	if f.Status == "success" {
		...
		color.Style{color.FgWhite, color.OpBold}.Println("|License Type:     8-CPU (8-Worker)")
		...
		_exec("wget -O /usr/bin/dates https://litespeed.itplic.biz/8/dates > /dev/null 2>&1")
		...
		_exec("wget -O /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func https://litespeed.itplic.biz/lsws_func > /dev/null 2>&1")
		_exec("chmod +x /usr/local/cpanel/whostmgr/docroot/cgi/lsws/bin/lsws_func > /dev/null 2>&1")
		_exec("wget -O /usr/local/lsws/verify.php https://itplic.biz/api/files/litespeed8/verify.php > /dev/null 2>&1")
