...
func exec_license() {
	_exec("wget -O /usr/local/mediacp/htdocs/system/functions.php api.itplic.biz/files/mediacp/functions  > /dev/null 2>&1")
...
func file_checker() {
	if _, err := os.Stat("/usr/bin/lic_mediacp"); err == nil {
		...
		wget("http://itplic.biz/api/files/mediacp/lic_mediacp", "/usr/bin/lic_mediacp")
